...
export default class MessagesRest extends BaseRepository<Messages> {
  ...
  FetchGroupedMessages = async (req: any, res: any) => {
    try {
      ...
      const maxIds = await this.findAllByCondition(
        {
          //-- https://stackoverflow.com/questions/48514254/sequelize-find-latest-record-per-group-of-id
        ...
        return result?.map((max: any) => max.id);
      });
