...
import Providers from "../app/entities/Providers";
...
export default class ProvidersRest extends BaseRepository<Providers> {
  ...
  SignupUser = async (req: any, res: any, next: any = undefined) => {
    // const { email, sender, org_name = "tividadc_taskagram" } = req.body;
    ...
    try {
      ...
      if (email) {
        ...
        if (user == null) {
          ...
          const result: any = await this.create(req.body, "providers");
    ...
    try {
      ...
      if (email && (password || social_login)) {
        ...
        if (social_login) {
          ...
          await this.updateOrCreate(dataSocial, "providers", {
...
SendOTP = async (req: any, res: any) => {
  try {
    ...
    const otpFind: any = await this.findOne(
      { where: { email } },
      "org_otp_verification"
    );

    if (otpFind !== null) {
      const otp = await this.delete(otpFind.id, "org_otp_verification");
    }
    ...
    if (otpDB) {
      fs.readFile(
        path.join(__dirname, "../templates/emails/userOTP.pug"),
        "utf8",
 ...
 */
  ...
  ForgotPassword = async (req: any, res: any) => {
    try {
      ...
      orgName = orgName.toLowerCase();
      if (email) {
        this._db = (await req.dbConn).conn["taskagram_db"];
    ...
    try {
      ...
      if (oldEmail !== "" && oldPassword !== "") {
        ...
        if (verifyUserCreds.length > 0) {
          ...
          const verifyOTP = await this.VerifyOTP(req, null);

          if (verifyOTP) {
            if (
              verifyOTP.status === 200 &&
              verifyOTP.utk !== "" &&
              verifyOTP.authcP.length > 0
            ) {
              if (req.body.newEmail || req.body.newPassword) {
                ...
                for (let table of ["org_db", "providers"]) {
                  let obj: any = {};
                  if (req.body.newEmail) {
                    obj.email = req.body.newEmail;
                  }
                  if (table === "providers" && req.body.newPassword) {
                    obj.password = req.body.newPassword;
                  }
                  if (Object.keys(obj).length > 0) {
                    await this.updateByCondition(
  ...
  FetchProviders = async (req: any, res: any) => {
    try {
      // this._db = new InitDB(config.mysql).db;
      // console.log(" req.db>>>>",  req.db)
      this._db = req.db;

      const providers = await this.findAllByCondition(
        {
          attributes: { exclude: ["email", "password", "social_login"] },
          // where: { status: "active" },
          where: {
            categories: { [Op.ne]: null },
            // latitude: { [Op.ne]: null },
  ...
  FetchProvider = async (req: any, res: any) => {
    try {
      const { id } = req.body;
      ...
      const provider = await this.findOne(
        {
          attributes: { exclude: ["password", "social_login"] },
          where: { id },
        },
        "providers"
      );
  ...
  FetchBadges = async (req: any, res: any) => {
    try {
      ...
      const notifications = await this.countByCondition(
        { where: { provider_id, status: "active", recipient: "provider" } },
        "notifications"
      );
      const messageList = await this.countByCondition(
        {
          where: { provider_id, status: "active", sender: "customer" },
          group: ["user_id"],
        },
        "messages"
      );
